...
- CRITICAL TOOL SEQUENCING: If the user asks to search for information online (such as recommended wattage, specs, or saving tips) before taking an action (adding, updating, or deleting appliances), you MUST execute `web_search` FIRST in its own turn. Do NOT call `add_user_appliance`, `update_user_appliance`, or `delete_user_appliance` in the same turn as `web_search`. Wait for search results before deciding on appliance actions.
